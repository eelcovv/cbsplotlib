...
class CBSHighChart:
    def __init__(
        ...
        if driver_version is not None:
            ...
            try:
                ...
                _logger.warning(err)
                raise ValueError(
                    f"Driver version {driver_version_short} is not defined. Please give full version name"
                )
        ...
        if start:
            ...
            if convert_to_html:
                if selenium is None:
                    _logger.warning(
                        "convert to html requested, but selenium is not installed. Skipping"
                    )
                else:
    ...
    def convert_json_to_html(self, json_input_file=None):
        ...
        try:
            ...
            _logger.warning(err)
            _logger.warning(
                f"Cannot get driver from path {self.driver}. Stop writing html here"
            )
            return
